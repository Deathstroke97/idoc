...
- Endpoints:
  ...
  - `GET /appointments`: list appointments; optional `user_phone` to filter.
  - `DELETE /appointments/{appointment_id}`: cancel appointment by id.
- Files:
...
- Pages:
  ...
  - `/appointments` page (“My appointments”): list/cancel appointments; supports filtering by `user_phone`.
...
- API client in `frontend/lib/api.ts` with base URL `NEXT_PUBLIC_API_BASE_URL` (defaults to `http://localhost:8000`). Uses `/clinics`, `/doctors`, `/make-appointmet`, and appointments endpoints.
